...
void EcouteCallback (IvyClientPtr app, void *data, int argc, char **argv){
    ...
    if(retour==0){
        IvySendMsg("Baloo type=OK");
    }else{
        IvySendMsg("Baloo type=ERREUR");
    }
...
int main (int argc, char**argv){
	/* initialisation */
	IvyInit ("Akela", "Debout louveteau\n", 0, 0, 0, 0);

	/* On ecoute et on traite les messages qui commencent par n'importe quoi */
	IvyBindMsg (EcouteCallback, 0, "^Akela type=(.*)_(.*) argument=(.*)");

	/* On ecoute et on traite les messages 'Bye' */
	IvyBindMsg (ByeCallback, 0, "^Akela Bye$");
