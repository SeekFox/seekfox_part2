...
public class ConfigScreen {
    ...
    public ConfigScreen(PApplet p){
        this.p = p;
        sliderNameI = new Text(p.width/2 - 80,p.height/5, "Nb Bits");
        sliderImage = new Slider(p.width/2 - 40,p.height/5,50,2,8,p);
        valConfigImage = new Text(p.width/2 + 20 ,p.height/5,(String.valueOf(sliderImage.getValue())));

        sliderNameAn = new Text(p.width/2 - 80,p.height * 2/5, "insert parameter name");
        sliderAudio_n = new Slider(p.width/2 - 40,p.height * 2/5,50,10,100,p);
        valConfigAudio_n = new Text(p.width/2 +20, p.height * 2/5 , String.valueOf(sliderAudio_n.getValue()));

        sliderNameAm = new Text(p.width/2 - 80, p.height *3/5 , "insert param audio");
        sliderAudio_m = new Slider(p.width/2 - 40,p.height * 3/5,50,500,5000,p);
        valConfigAudio_m = new Text(p.width + 20,p.height* 3/5, String.valueOf(sliderAudio_m.getValue()));

        sliderNameT = new Text(p.width/2 - 80, p.height * 4/5, "insert param text");
        sliderTexte  = new Slider(p.width/2 - 40,p.height * 4/5,50,3,10,p);
        valConfigTexte = new Text(p.width + 20,p.height * 4/5, String.valueOf(sliderTexte.getValue()));
